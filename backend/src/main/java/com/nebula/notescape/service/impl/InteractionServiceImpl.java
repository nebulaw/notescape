...
public class InteractionServiceImpl extends BaseService implements IInteractionService {
  ...
  public ApiResponse getWatchlistByUserId(Long userId, Integer page, Integer size, String[] sort) {
    ...
    return ApiResponse.builder()
        .data(watchDao
            .getWatchPageByUserId(userId, createPageable(page, size, sort)).stream()
            .map(Watch::getMovie)
            .toList())
        .status(HttpStatus.OK)
        .build();
  ...
  public ApiResponse getWishlistByUserId(Long userId, Integer page, Integer size, String[] sort) {
    ...
    return ApiResponse.builder()
        .data(wishDao
            .getWishPageByUserId(userId, createPageable(page, size, sort)).stream()
            .map(Wish::getMovie)
            .toList())
        .status(HttpStatus.OK)
        .build();
  ...
  @Override
  public ApiResponse watch(Long userId, Long movieTmdbId) {
    validateId(userId);
    validateId(movieTmdbId);

    WatchKey watchKey = new WatchKey(userId, movieTmdbId);
    ...
    watchDao
        ...
        .ifPresentOrElse(watch -> {
          ...
          watchDao.save(watch);
        }, () -> watchDao.save(new Watch(
            watchKey,
            userDao.getById(userId).orElseThrow(() ->
                new ApiException(Exceptions.USER_NOT_FOUND)),
            movieDao.getByTmdbId(movieTmdbId).orElseThrow(() ->
                new ApiException(Exceptions.MOVIE_NOT_FOUND)))));
  ...
  @Override
  public ApiResponse wish(Long userId, Long movieTmdbId) {
    validateId(userId);
    validateId(movieTmdbId);

    wishDao
        .getById(new WishKey(userId, movieTmdbId))
        .ifPresentOrElse(wish -> {
          wish.setRecordState(RecordState.ACTIVE);
          wish.setUpdateDate(LocalDateTime.now());
          wishDao.save(wish);
        }, () -> wishDao.save(new Wish(
            new WishKey(userId, movieTmdbId),
            userDao.getById(userId).orElseThrow(() ->
                new ApiException(Exceptions.USER_NOT_FOUND)),
            movieDao.getByTmdbId(movieTmdbId).orElseThrow(() ->
                new ApiException(Exceptions.MOVIE_NOT_FOUND)))));
